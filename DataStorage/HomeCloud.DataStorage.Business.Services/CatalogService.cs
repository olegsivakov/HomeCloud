namespace HomeCloud.DataStorage.Business.Services
{
	...
	public class CatalogService : ICatalogService
	{
		...
		public async Task<ServiceResult<Catalog>> UpdateCatalogAsync(Catalog catalog)
		{
			...
			this.processor.CreateDataHandler<IDataCommandHandler>()
				.CreateAsyncCommand<IFileSystemProvider>(updateCatalogFunction, null)
				.CreateAsyncCommand<IAggregationDataProvider>(updateCatalogFunction, null);
		...
		/// </returns>
		public async Task<ServiceResult<IEnumerable<Catalog>>> GetCatalogsAsync(Guid parentID, int offset = 0, int limit = 20)
		{
			ServiceResult<Catalog> serviceResult = await this.GetCatalogAsync(parentID);
			if (!serviceResult.IsSuccess)
			{
				return new ServiceResult<IEnumerable<Catalog>>(Enumerable.Empty<Catalog>())
				{
			...
			return new ServiceResult<IEnumerable<Catalog>>(catalogs);
		}
